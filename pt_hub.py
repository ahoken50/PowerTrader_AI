...
class PowerTraderHub(tk.Tk):
    ...
    def open_settings_dialog(self) -> None:
        ...
        # --- Bitget API setup (writes b_key.txt, b_secret.txt, b_pass.txt) ---
        def _api_paths() -> Tuple[str, str, str]:
            ...
            pass_path = os.path.join(self.project_dir, "b_pass.txt")
        ...
        def _refresh_api_status() -> None:
            ...
            missing = []
            if not k: missing.append("b_key.txt (API Key)")
            if not s: missing.append("b_secret.txt (Secret Key)")
            if not p: missing.append("b_pass.txt (Passphrase)")
            if missing:
        ...
        def _clear_api_files() -> None:
            key_path, secret_path, pass_path = _api_paths()
            if not messagebox.askyesno("Delete credentials?", "This deletes API keys. Sure?"): return
            try:
                if os.path.isfile(key_path): os.remove(key_path)
                if os.path.isfile(secret_path): os.remove(secret_path)
                if os.path.isfile(pass_path): os.remove(pass_path)
            except Exception as e:
                messagebox.showerror("Error", f"Could not delete: {e}")
                return
            _refresh_api_status()
            messagebox.showinfo("Success", "Deleted Bitget credentials.")

        def _open_bitget_api_wizard() -> None:
            import webbrowser
            import base64
            import hmac
            import hashlib
            ...
            intro = (
                "Set up Bitget API Credentials (V2):\n"
                "1. Log in to Bitget.\n"
                "2. Create a System-generated API Key with Spot Trading & Read-only permissions.\n"
                "3. Note down the API Key, Secret Key, and Passphrase.\n"
                "4. Paste them here to save locally."
            )
            ttk.Label(viewport, text=intro, justify="left").pack(anchor="w", pady=(0, 10))
            
            ttk.Button(viewport, text="Open Bitget API Management", command=lambda: webbrowser.open("https://www.bitget.com/account/api-keys")).pack(anchor="w", pady=(0, 10))

            form = ttk.Frame(viewport)
            form.pack(fill="x", pady=10)
            form.columnconfigure(1, weight=1)

            existing_k, existing_s, existing_p = _read_api_files()
            
            ttk.Label(form, text="API Key:").grid(row=0, column=0, sticky="w", pady=5)
            api_key_var = tk.StringVar(value=existing_k)
            ttk.Entry(form, textvariable=api_key_var).grid(row=0, column=1, sticky="ew", pady=5)

            ttk.Label(form, text="Secret:").grid(row=1, column=0, sticky="w", pady=5)
            secret_key_var = tk.StringVar(value=existing_s)
            ttk.Entry(form, textvariable=secret_key_var).grid(row=1, column=1, sticky="ew", pady=5)

            ttk.Label(form, text="Passphrase:").grid(row=2, column=0, sticky="w", pady=5)
            pass_var = tk.StringVar(value=existing_p)
            ttk.Entry(form, textvariable=pass_var, show="*").grid(row=2, column=1, sticky="ew", pady=5)
            ...
            def test_creds():
                ...
                if not requests:
                    messagebox.showerror("Error", "Requests library needed")
                    return
                if not ak or not sk or not pw:
                    messagebox.showerror("Error", "All 3 fields required")
                    return
                ts = str(int(time.time() * 1000))
                msg = ts + "GET" + "/api/v2/spot/account/assets"
                sig = base64.b64encode(hmac.new(sk.encode('utf-8'), msg.encode('utf-8'), hashlib.sha256).digest()).decode('utf-8')
                headers = {"ACCESS-KEY": ak, "ACCESS-SIGN": sig, "ACCESS-TIMESTAMP": ts, "ACCESS-PASSPHRASE": pw, "locale": "en-US"}
                try:
                    r = requests.get("https://api.bitget.com/api/v2/spot/account/assets", headers=headers, timeout=10).json()
                    if str(r.get("code")) == "00000":
                        messagebox.showinfo("Success", "Bitget API connected successfully!")
                    else:
                        messagebox.showerror("API Error", str(r.get("msg")))
                except Exception as e:
                    messagebox.showerror("Network Error", str(e))

            ttk.Button(form, text="Test Connection (Read Only)", command=test_creds).grid(row=3, column=0, columnspan=2, pady=10)
            ...
            def save_creds():
                ...
                k, s, p = _api_paths()
                try:
                    with open(k, "w", encoding="utf-8") as f: f.write(ak)
                    with open(s, "w", encoding="utf-8") as f: f.write(sk)
                    with open(p, "w", encoding="utf-8") as f: f.write(pw)
                    _refresh_api_status()
            ...
            btns = ttk.Frame(viewport)
            btns.pack(fill="x", pady=10)
            ttk.Button(btns, text="Save", command=save_creds).pack(side="left")
            ttk.Button(btns, text="Close", command=wiz.destroy).pack(side="left", padx=10)

        ttk.Label(frm, text="Bitget V2 API:").grid(row=r, column=0, sticky="w", padx=(0, 10), pady=6)
        ...
        ttk.Button(api_row, text="Setup Wizard", command=_open_bitget_api_wizard).grid(row=0, column=1, sticky="e", padx=(10, 0))
